...
def need_data():
    if st.session_state.df is None:
        ...
        if st.button("Go to Data"):
            st.session_state.nav="📁 Data";st.rerun()
        st.stop()
...
if page=="🏠 Workspace":
    ...
    if st.session_state.df is None:
        ...
        with c1:
            ...
            if st.button("Open Data workspace",type="primary",use_container_width=True):
                st.session_state.nav="📁 Data";st.rerun()
        with c2:
        ...
        if b1.button("📊 Explore data",use_container_width=True):
            st.session_state.nav="📊 Explore";st.rerun()
        if b2.button("🧮 Build a model",type="primary",use_container_width=True):
            st.session_state.nav="🧮 Model Studio";st.rerun()
        if b3.button("🧪 Open Research Lab",use_container_width=True):
            st.session_state.nav="🧪 Research Lab";st.rerun()
